...
def train(fps, args):
  ...
  if audio.shape[0] < args.data_slice_len:
    audio = np.pad(audio, (0, args.data_slice_len - audio.shape[0]), 'constant')
  adv_input = tf.constant(audio[:args.data_slice_len], dtype=np.float32) + args.adv_magnitude * tf.reshape(G_z, G_z.get_shape().as_list()[:-1])


  # Calculate MFCCs
  spectrograms = tf.abs(tf.signal.stft(adv_input, frame_length=320, frame_step=160))
  linear_to_mel_weight_matrix = tf.signal.linear_to_mel_weight_matrix(40, spectrograms.shape[-1].value, fs, 20, 4000)
  mel_spectrograms = tf.tensordot(spectrograms, linear_to_mel_weight_matrix, 1)
  mel_spectrograms.set_shape(spectrograms.shape[:-1].concatenate(linear_to_mel_weight_matrix.shape[-1:]))
  log_mel_spectrograms = tf.math.log(mel_spectrograms + 1e-6)
  mfccs = tf.expand_dims(tf.signal.mfccs_from_log_mel_spectrograms(log_mel_spectrograms)[-1, :99, :40], -1)
  ...
  with tf.gfile.FastGFile(args.adv_model, 'rb') as f:
    ...
    with tf.variable_scope('Speech'):
      adv_logits, = tf.import_graph_def(graph_def, input_map={'Mfcc:0': mfccs}, return_elements=['add_2:0'])
  ...
  adv_targets = tf.one_hot(tf.constant([adv_index] * args.train_batch_size, dtype=tf.int32), len(adv_labels))
  ...
  adv_loss = tf.reduce_mean(tf.maximum(0.0, adv_others_logit - adv_target_logit + args.adv_confidence))

  # Summarize audios
  tf.summary.audio('adv_input', adv_input, fs, max_outputs=args.adv_max_outputs)
  ...
  # Create training ops
  G_train_op = G_opt.minimize(G_loss + args.adv_lambda * adv_loss, var_list=G_vars,
...
if __name__ == '__main__':
  ...
  adv_args.add_argument('--adv_input', type=str, help='Original audio for generating adversarial example')
  adv_args.add_argument('--adv_model', type=str, help='Path of the target model (.ckpt)')
  adv_args.add_argument('--adv_label', type=str, help='Path of the target labels (.txt)')
  adv_args.add_argument('--adv_target', type=str, help='Target command')
  adv_args.add_argument('--adv_magnitude', type=float, help='Parameter specifing how much the perturbation be diminished')
  adv_args.add_argument('--adv_confidence', type=float, help='Parameter specifing how strong the adversarial example should be')
  ...
  parser.set_defaults(
    adv_input=None,
    adv_model='./model/conv_actions_frozen.pb',
    adv_label='./model/conv_actions_labels.txt',
    adv_target='stop',
    adv_confidence=0,
    adv_magnitude=1.0,
    adv_lambda=0.01,
