...
flags.DEFINE_float('adv_alpha', 0.001,
                   'The parameter to adjust the magnitude of the gradient in PGPE.')
...
def main(_):
  ...
  # Define parameters for the image composition
  adv_mu = tf.Variable(np.zeros((3, ), dtype=np.float32), name='mu')
  adv_sigma = tf.Variable(0.3 * np.ones((3, ), dtype=np.float32), name='sigma')

  # Sample variables for the image composition
  adv_dist = tf.distributions.Normal(loc=0.0, scale=adv_sigma)
  adv_sample_count = int(FLAGS.batch_size / 2)
  adv_epsilon = adv_dist.sample((adv_sample_count, ))
  adv_sample = tf.clip_by_value(tf.concat([adv_mu + adv_epsilon, adv_mu - adv_epsilon], 0), -1, 1)

  # Convert to transformation parameters
  adv_theta = adv_sample[:, 0] * math.pi
  adv_dx = adv_sample[:, 1] * image_pad
  adv_dy = adv_sample[:, 2] * image_pad

  with tf.name_scope('composition'):
    tf.summary.scalar('composition_theta_mu', adv_mu[0])
    tf.summary.scalar('composition_theta_sigma', adv_sigma[0])
    tf.summary.scalar('composition_dx_mu', adv_mu[1])
    tf.summary.scalar('composition_dx_sigma', adv_sigma[1])
    tf.summary.scalar('composition_dy_mu', adv_mu[2])
    tf.summary.scalar('composition_dy_sigma', adv_sigma[2])
    tf.summary.histogram('composition_theta', adv_theta)
    tf.summary.histogram('composition_dx', adv_dx)
    tf.summary.histogram('composition_dy', adv_dy)
  ...
  def generator_loss(gan_model, *args, **kwargs):
    ...
    with tf.name_scope('generator_loss'):
      ...
      adv_loss_individual = tf.maximum(0.0, adv_others_logit - adv_target_logit + FLAGS.adv_confidence)
      adv_loss = tf.reduce_mean(adv_loss_individual)
    ...
    # Update parameters using PGPE
    reward, baseline = -adv_loss_individual, -adv_loss

    r_t = (reward[:adv_sample_count] - reward[adv_sample_count:]) / (0 - reward[:adv_sample_count] - reward[adv_sample_count:])
    r_s = ((reward[:adv_sample_count] + reward[adv_sample_count:]) / 2 - baseline) / (0 - baseline)

    mu_grad = 2 * tf.matmul(tf.transpose(adv_epsilon), tf.expand_dims(r_t, 1))
    sigma_grad = tf.matmul(tf.transpose(tf.div(tf.square(adv_epsilon) - tf.square(adv_sigma), adv_sigma)), tf.expand_dims(r_s, 1))

    with tf.variable_scope('Generator', reuse=True):
      assign_mu = tf.assign(adv_mu, adv_mu + FLAGS.adv_alpha * mu_grad[:, 0], name='MagicAssignMu')
      assign_sigma = tf.assign(adv_sigma, adv_sigma + FLAGS.adv_alpha * sigma_grad[:, 0], name='MagicAssignSigma')
      tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, assign_mu)
      tf.add_to_collection(tf.GraphKeys.UPDATE_OPS, assign_sigma)
  ...
  gan_saver = tf.train.Saver(dict((var.name.replace(':0', ''), var) for var in gan_vars if 'Magic' not in var.name))
